...
async function main() {
  ...
  const InterestRateModel = await ethers.getContractFactory("VariableInterestRateModel")
  const dummyLendingPool = ethers.ZeroAddress
  ...
  const lendingPool = await LendingPool.deploy(await interestModel.getAddress())
  await lendingPool.waitForDeployment()
  console.log("LendingPool:", await lendingPool.getAddress())

  // Update interest model with correct lending pool address
  await interestModel.connect(deployer).transferOwnership(await lendingPool.getAddress())

  // Deploy flash loan receiver
  const FlashLoanReceiver = await ethers.getContractFactory("FlashLoanReceiver")
  const flashLoanReceiver = await FlashLoanReceiver.deploy(await lendingPool.getAddress())
